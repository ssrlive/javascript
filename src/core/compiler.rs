...
impl<'gc> Compiler<'gc> {
    ...
    pub fn compile(mut self, statements: &[Statement]) -> Result<Chunk<'gc>, JSError> {
        ...
        let mut remaining_non_function = statements
            .iter()
            .filter(|stmt| {
                !matches!(*stmt.kind, StatementKind::FunctionDeclaration(..)) && !is_exported_fn_decl(stmt) && !is_export_default_fn(stmt)
            })
            .count();
        ...
        // non-empty completion should be preserved.
        {
            ...
            if non_fn_stmts.len() >= 2
                && let Some(last) = non_fn_stmts.last()
            {
        ...
        for stmt in statements.iter() {
            if matches!(*stmt.kind, StatementKind::FunctionDeclaration(..)) || is_exported_fn_decl(stmt) || is_export_default_fn(stmt) {
                continue;
            }
        ...
        match &*stmt.kind {
            ...
            StatementKind::For(for_stmt) => {
                ...
                let is_lexical_init = if let Some(init) = &for_stmt.init {
                    matches!(
                        ...
                        StatementKind::Let(_)
                            ...
                            | StatementKind::ConstDestructuringObject(_, _)
                    )
                } else {
                    false
                };
                ...
                let let_slots: Vec<u8> = if let Some(init) = &for_stmt.init {
                    let mut names: Vec<String> = match &*init.kind {
                        ...
                        StatementKind::LetDestructuringArray(elements, _) | StatementKind::ConstDestructuringArray(elements, _) => {
                            let mut out = Vec::new();
                            for elem in elements {
                                Self::collect_destructuring_binding_names(elem, &mut out);
                            }
                            out
                        }
                        StatementKind::LetDestructuringObject(elements, _) | StatementKind::ConstDestructuringObject(elements, _) => {
                            let mut out = Vec::new();
                            for elem in elements {
                                Self::collect_object_destructuring_binding_names(elem, &mut out);
                            }
                            out
                        }
                        _ => Vec::new(),
            ...
            StatementKind::ForIn(decl_kind, var_name, obj_expr, body) => {
                ...
                if let Some(ref key_name) = current_key_name {
                    ...
                    self.locals.push(var_name.clone());
                    // Per-iteration fresh binding: create a new upvalue cell so
                    // closures from each iteration capture independent values.
                    if is_lexical_binding {
                        let var_slot = (self.locals.len() - 1) as u8;
                        self.chunk.write_opcode(Opcode::ReboxLocal);
                        self.chunk.write_byte(var_slot);
                        // ReboxLocal may copy from the old cell; overwrite with
                        // the current iteration value sitting on the stack.
                        self.chunk.write_opcode(Opcode::SetLocal);
                        self.chunk.write_byte(var_slot);
                    }
                }
            ...
            StatementKind::ForInDestructuringArray(decl_kind, elements, obj_expr, body) => {
                let saved_cv = self.completion_var.clone();
                if is_last {
                    self.setup_completion_var();
                }
                self.compile_for_in_destructuring_array(decl_kind.as_ref(), elements, obj_expr, body)?;
                if is_last {
                    self.emit_load_completion();
                } else {
                    self.completion_var = saved_cv;
            ...
            StatementKind::ForInDestructuringObject(decl_kind, elements, obj_expr, body) => {
                let saved_cv = self.completion_var.clone();
                if is_last {
                    self.setup_completion_var();
                }
                self.compile_for_in_destructuring_object(decl_kind.as_ref(), elements, obj_expr, body)?;
                if is_last {
                    self.emit_load_completion();
                } else {
                    self.completion_var = saved_cv;
            ...
            StatementKind::ForOfExpr(lhs_expr, iterable_expr, body) | StatementKind::ForAwaitOfExpr(lhs_expr, iterable_expr, body) => {
                // Lazy iteration with IteratorClose on early exit, assigns to expression LHS
                let saved_cv = self.completion_var.clone();
                if is_last {
                    self.setup_completion_var();
                }
                ...
                let fid = self.forin_counter;
                self.forin_counter = self.forin_counter.saturating_add(1);
                let iter_var = format!("__forofIter_{}__", fid);
                let next_fn_var = format!("__forofNext_{}__", fid);

                if is_for_await {
                    // For-await-of: keep using __forOfValues for async iteration
                    self.compile_expr(&Expr::Call(
                        Box::new(Expr::Var(INTERNAL_FOROF_HELPER.to_string(), None, None)),
                        vec![iterable_expr.clone()],
                    ))?;
                    ...
                    // Synchronous for-of: lazy iteration with IteratorClose on early exit
                    // Get iterator
                    self.compile_expr(&Expr::Call(
                        Box::new(Expr::Var(INTERNAL_GETITER_HELPER.to_string(), None, None)),
                        vec![iterable_expr.clone()],
                    ...
                    // Loop start
                    let loop_start = self.chunk.code.len();
                    let mut lctx = self.make_loop_context(loop_start);
                    lctx.for_of_iter_var = Some(iter_var.clone());
                    self.loop_stack.push(lctx);

                    // Call next_fn.call(iter) → result
                    self.emit_helper_get(&iter_var);
                    self.emit_helper_get(&next_fn_var);
                    self.emit_call_opcode(0, 0x80);

                    // IteratorResult type check
                    self.chunk.write_opcode(Opcode::AssertIterResult);
                    ...
                    // SetupTry for iterator close on throw
                    self.chunk.write_opcode(Opcode::SetupTry);
                    let try_catch_placeholder = self.chunk.code.len();
                    self.chunk.write_u32(0xffff_ffff);
                    let exc_binding = self
                        .chunk
                        .add_constant(Value::String(crate::unicode::utf8_to_utf16("__forofExc__")));
                    ...
                    // TeardownTry — normal completion
                    self.chunk.write_opcode(Opcode::TeardownTry);
                    let jump_over_catch = self.emit_jump(Opcode::Jump);

                    // Catch: close iterator, re-throw
                    let catch_ip = self.chunk.code.len();
                    self.chunk.code[try_catch_placeholder] = (catch_ip & 0xff) as u8;
                    self.chunk.code[try_catch_placeholder + 1] = ((catch_ip >> 8) & 0xff) as u8;
                    self.chunk.code[try_catch_placeholder + 2] = ((catch_ip >> 16) & 0xff) as u8;
                    ...
                    // Exit: pop result object
                    self.patch_jump(exit_jump);
                    self.chunk.write_opcode(Opcode::Pop);

                    let lctx = self.loop_stack.pop().unwrap();
                    for bp in lctx.break_patches {
                        self.patch_jump(bp);
                    }
                }
            ...
            StatementKind::Export(specifiers, declaration, _source) => {
                ...
                for spec in specifiers {
                    if let crate::core::statement::ExportSpecifier::Default(expr) = spec {
                        ...
                        // Per spec §15.2.3.11, anonymous function/generator gets name "default"
                        ...
                        // Track function IPs before compiling to find the newly created function
                        let pre_fn_ips: Vec<usize> = self
                            .chunk
                            .constants
                            .iter()
    ...
    fn compile_for_in_destructuring_array(
        ...
        // TDZ: push Uninitialized for all bound names before evaluating obj_expr
        if is_lexical {
            for name in &bound_names {
                if !self.locals[saved_locals..].iter().any(|l| l == name) {
                    let uninit = self.chunk.add_constant(Value::Uninitialized);
    ...
    fn compile_for_in_destructuring_object(
        ...
        // TDZ: push Uninitialized before evaluating obj_expr
        if is_lexical {
            for name in &bound_names {
                if !self.locals[saved_locals..].iter().any(|l| l == name) {
                    let uninit = self.chunk.add_constant(Value::Uninitialized);
