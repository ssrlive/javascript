...
fn define_function_length<'gc>(ctx: &GcContext<'gc>, func_obj: &JSObjectDataPtr<'gc>, length: usize) -> Result<(), JSError> {
    let desc = create_descriptor_object(ctx, &Value::Number(length as f64), false, false, true)?;
    crate::js_object::define_property_internal(ctx, func_obj, "length", &desc)?;
    Ok(())
}
...
impl<'gc> PropertyDescriptor<'gc> {
    ...
    pub fn to_object(&self, ctx: &GcContext<'gc>) -> Result<JSObjectDataPtr<'gc>, JSError> {
        ...
        if self.get.is_some() || self.set.is_some() {
            if let Some(g) = &self.get {
                object_set_key_value(ctx, &desc, "get", g)?;
            }
            if let Some(s) = &self.set {
                object_set_key_value(ctx, &desc, "set", s)?;
            }
            object_set_key_value(ctx, &desc, "enumerable", &Value::Boolean(self.enumerable.unwrap_or(false)))?;
            object_set_key_value(ctx, &desc, "configurable", &Value::Boolean(self.configurable.unwrap_or(false)))?;
            return Ok(desc);
        }

        // Otherwise, data descriptor behavior
        let val = self.value.clone().unwrap_or(Value::Undefined);
        object_set_key_value(ctx, &desc, "value", &val)?;
        object_set_key_value(ctx, &desc, "writable", &Value::Boolean(self.writable.unwrap_or(false)))?;
        object_set_key_value(ctx, &desc, "enumerable", &Value::Boolean(self.enumerable.unwrap_or(false)))?;
        object_set_key_value(ctx, &desc, "configurable", &Value::Boolean(self.configurable.unwrap_or(false)))?;
        Ok(desc)
    }
...
pub fn create_descriptor_object<'gc>(
    ...
    PropertyDescriptor::new_data(value, writable, enumerable, configurable).to_object(ctx)
}
...
pub(crate) fn build_property_descriptor<'gc>(
        ...
        && s != "then"
    {
        ...
        if let (Some(module_path), Some(cache_env)) = (module_path, cache_env)
            && let Ok(Value::Object(exports_obj)) = crate::js_module::load_module(ctx, module_path.as_str(), None, Some(cache_env))
            && let Some(v) = object_get_key_value(&exports_obj, s)
        {
            let resolved = v.borrow().clone();
            let _ = crate::core::object_set_key_value(ctx, obj, s.as_str(), &resolved);
        }
    }
                ...
                if let Some(g) = getter {
                    match &*g.clone() {
                        Value::Getter(body, captured_env, _home) => {
                            ...
                            let _ = crate::core::set_internal_prototype_from_constructor(ctx, &func_obj, captured_env, "Function");
                            ...
                            let _ = define_function_length(ctx, &func_obj, 0);
                            ...
                            let desc = crate::core::create_descriptor_object(ctx, &Value::from(&full_name), false, false, true).unwrap();
                            let _ = crate::js_object::define_property_internal(ctx, &func_obj, "name", &desc);
                ...
                if let Some(s) = setter {
                    match &*s.clone() {
                        Value::Setter(params, body, captured_env, _home) => {
                            ...
                            let closure_val = Value::Closure(Gc::new(ctx, closure_data));
                            func_obj.borrow_mut(ctx).set_closure(Some(new_gc_cell_ptr(ctx, closure_val)));
                            let _ = define_function_length(ctx, &func_obj, compute_function_length(params));
                            let prop_name = match key {
                                PropertyKey::String(s) => s.clone(),
                                PropertyKey::Symbol(sd) => {
                                    if let Some(desc) = sd.description() {
...
pub fn validate_descriptor_for_define<'gc>(_ctx: &GcContext<'gc>, pd: &PropertyDescriptor<'gc>) -> Result<(), JSError> {
