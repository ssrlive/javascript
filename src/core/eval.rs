...
pub(crate) fn to_number<'gc>(val: &Value<'gc>) -> Result<f64, EvalError<'gc>> {
    match val {
        ...
        Value::String(s) => Ok(crate::js_number::string_to_f64(&utf16_to_utf8(s)).unwrap_or(f64::NAN)),
        Value::BigInt(_) => Err(raise_type_error!("Cannot convert BigInt to number").into()),
        Value::Symbol(_) => Err(raise_type_error!("Cannot convert Symbol to number").into()),
...
fn loose_equal<'gc>(mc: &MutationContext<'gc>, l: Value<'gc>, r: Value<'gc>, env: &JSObjectDataPtr<'gc>) -> Result<bool, EvalError<'gc>> {
    match (l, r) {
        ...
        (Value::String(s), Value::Number(n)) | (Value::Number(n), Value::String(s)) => {
            let sn = crate::js_number::string_to_f64(&utf16_to_utf8(&s)).unwrap_or(f64::NAN);
            Ok(n == sn)
        }
        ...
        match inner {
            DestructuringElement::Property(key, boxed) => match &**boxed {
                DestructuringElement::Variable(name, default_expr) => {
                    ...
                    env_set_recursive(mc, &target_env, name, prop_val.clone())?;
                    if used_default {
                        maybe_set_function_name_for_default(mc, name, default_expr.as_deref().unwrap(), &prop_val)?;
                    }
                    excluded_keys.push(PropertyKey::String(key.clone()));
        ...
        StatementKind::LetDestructuringArray(pattern, expr) | StatementKind::ConstDestructuringArray(pattern, expr) => {
            ...
            if has_nested {
                if let Value::Object(obj) = &val {
                    if is_array(mc, obj) || is_typedarray(obj) {
                        // For fixed-length TypedArrays, ensure the entire view still fits in the buffer.
                        if is_typedarray(obj) {
                            ensure_typedarray_in_bounds(mc, env, Some(stmt.line), Some(stmt.column), obj)?;
                        }
                        let is_const = matches!(*stmt.kind, StatementKind::ConstDestructuringArray(_, _));
                        bind_array_inner_for_letconst(mc, env, pattern, obj, is_const, Some(stmt.line), Some(stmt.column))?;
                        *last_value = Value::Undefined;
                        return Ok(None);
                    } else {
                        return Err(raise_eval_error!("Cannot destructure non-array value").into());
                    }
                } else {
                    return Err(raise_eval_error!("Cannot destructure non-array value").into());
                }
            ...
            // Debug: log the kind of value we're destructuring
            if let Value::Object(o) = &val {
                log::trace!(
                    "let simple destructure: target is Object is_array={} is_typedarray={}",
                    is_array(mc, o),
                    is_typedarray(o)
                );
            } else {
                log::trace!("let simple destructure: target is NOT an object: {:?}", value_to_string(&val));
            }
            for (i, elem) in pattern.iter().enumerate() {
                if let DestructuringElement::Variable(name, default_expr) = elem {
                    ...
                    let mut elem_val = Value::Undefined;
                    if let Value::Object(obj) = &val
                        && (is_array(mc, obj) || is_typedarray(obj))
                    {
                        if is_typedarray(obj) {
                            // Diagnostic trace to confirm we check bounds for each element
                            ...
                            {
                                log::trace!(
                                    "simple destructure: idx={} ta.byte_offset={} ta.length_tracking={}",
                                    i,
                                    ta.byte_offset,
                                    ta.length_tracking
                                );
                        ...
                        elem_val = get_array_like_element(mc, env, obj, i)?;
                    }
        ...
        StatementKind::VarDestructuringArray(pattern, expr) => {
            ...
            if has_nested {
                if let Value::Object(obj) = &val {
                    if is_array(mc, obj) || is_typedarray(obj) {
                        ...
                        bind_array_inner_for_var(mc, env, pattern, obj, None, None)?;
                        *last_value = Value::Undefined;
                        return Ok(None);
                    } else {
                        return Err(raise_eval_error!("Cannot destructure non-array value").into());
            ...
            for (i, elem) in pattern.iter().enumerate() {
                match elem {
                    ...
                    DestructuringElement::Empty => {}
                    DestructuringElement::NestedArray(inner, inner_default) => {
                        let mut elem_val = Value::Undefined;
                        if let Value::Object(obj) = &val
                            && is_array(mc, obj)
                            && let Some(cell) = object_get_key_value(obj, i)
                        ...
                        if let Value::Object(oarr) = &elem_val {
                            bind_array_inner_for_var(mc, env, inner, oarr, None, None)?;
                        }
                    }
                    _ => {
                        return Err(raise_syntax_error!("Nested array destructuring not implemented").into());
            ...
            for elem_opt in elements.iter() {
                ...
                if let Some(elem_expr) = elem_opt {
                    match elem_expr {
                        Expr::Assign(boxed_lhs, boxed_default) => {
                            if matches!(value, Value::Undefined) {
                                    ...
                                    Ok(dv) => {
                                        ...
                                        let assign_res = match precomputed_temp.take() {
                                            Some(TargetTemp::Var(_)) => {
                                                evaluate_assign_target_with_value(mc, env, boxed_lhs, dv.clone()).map(|_| ())
                                            }
                                            Some(temp) => put_temp(mc, env, temp, dv.clone()),
                                            None => evaluate_assign_target_with_value(mc, env, boxed_lhs, dv.clone()).map(|_| ()),
                                        };
                                ...
                                let assign_res = if let Some(temp) = precomputed_temp.take() {
                                    put_temp(mc, env, temp, value.clone())
                                } else {
                                    evaluate_assign_target_with_value(mc, env, boxed_lhs, value.clone()).map(|_| ())
                                };
                                if let Err(e) = assign_res {
                                    let closed = iterator_close_on_error(mc, env, &iter_obj, e);
                                    clear_pending_iterator()?;
                        ...
                        other => {
                            if let Some(temp) = precomputed_temp.take() {
                                if let Err(e) = put_temp(mc, env, temp, value.clone()) {
                                    let closed = iterator_close_on_error(mc, env, &iter_obj, e);
                                    clear_pending_iterator()?;
                                    return Err(closed);
                                }
                            } else if let Err(e) = evaluate_assign_target_with_value(mc, env, other, value.clone()) {
                                let closed = iterator_close_on_error(mc, env, &iter_obj, e);
                                clear_pending_iterator()?;
                                return Err(closed);
                            }
            ...
            match target_expr {
                Expr::Assign(boxed_lhs, boxed_default) => {
                    if matches!(rhs_value, Value::Undefined) {
                        ...
                        match precomputed_temp.take() {
                            Some(TargetTemp::Var(_)) => {
                                evaluate_assign_target_with_value(mc, env, boxed_lhs, dv.clone())?;
                            }
                            Some(temp) => {
                                put_temp(mc, env, temp, dv.clone())?;
                            }
                            None => {
                                evaluate_assign_target_with_value(mc, env, boxed_lhs, dv.clone())?;
                            }
                        ...
                        put_temp(mc, env, temp, rhs_value.clone())?;
                    } else {
                        evaluate_assign_target_with_value(mc, env, boxed_lhs, rhs_value.clone())?;
                    }
                }
                Expr::Var(_, _, _) => {
                    if let Some(temp) = precomputed_temp.take() {
                ...
                other => {
                    if let Some(temp) = precomputed_temp.take() {
                        ...
                        evaluate_assign_target_with_value(mc, env, other, rhs_value.clone())?;
                    }
                }
            }
        }
    ...
    match target {
                ...
                for elem_opt in elements.iter() {
                    ...
                    if let Some(elem_expr) = elem_opt {
                        match elem_expr {
                            ...
                            Expr::Assign(_, _) => {
                                evaluate_assign_target_with_value(mc, env, elem_expr, value.clone())?;
                            }
                            other => {
                                evaluate_assign_target_with_value(mc, env, other, value.clone())?;
                            }
                        }
                    }
                }
                    ...
                    for (i, elem_opt) in elements.iter().enumerate() {
                        ...
                        if let Some(elem_expr) = elem_opt {
                            match elem_expr {
                                ...
                                Expr::Assign(_, _) => {
                                    evaluate_assign_target_with_value(mc, env, elem_expr, val_at.clone())?;
                                }
                                Expr::Spread(spread_expr) => {
                                    let rest_obj = crate::js_array::create_array(mc, env)?;
                                    let len = if let Some(len_cell) = object_get_key_value(&obj, "length") {
        ...
        Expr::Object(properties) => {
            ...
            for (key_expr, target_expr, is_spread) in properties.iter() {
                ...
                match target_expr {
                    Expr::Assign(_, _) => {
                        evaluate_assign_target_with_value(mc, env, target_expr, rhs_value.clone())?;
                    }
                    Expr::Var(name, _, _) => {
                        env_set_recursive(mc, env, name, rhs_value.clone())?;
                    }
    ...
    match target {
            ...
            if let Some(iter_obj) = iterator {
                ...
                for elem_opt in elements.iter() {
                    ...
                    if let Some(elem_expr) = elem_opt {
                        match elem_expr {
                            Expr::Var(name, _, _) => {
                                env_set(mc, env, name, value.clone())?;
                            }
                            Expr::Assign(_, _) => {
                                evaluate_binding_target_with_value(mc, env, elem_expr, value.clone())?;
                            }
                            other => {
                                evaluate_binding_target_with_value(mc, env, other, value.clone())?;
                            }
                        }
                    }
                }
                ...
                Ok(val)
            } else if let Value::Object(obj) = rhs {
                for (i, elem_opt) in elements.iter().enumerate() {
                    let val_at = if is_typedarray(&obj) {
                        get_property_with_accessors(mc, env, &obj, i)?
                    } else if let Some(cell) = object_get_key_value(&obj, i.to_string()) {
                        cell.borrow().clone()
                    } else {
                        Value::Undefined
                    ...
                    if let Some(elem_expr) = elem_opt {
                        match elem_expr {
                            Expr::Var(name, _, _) => {
                                env_set(mc, env, name, val_at.clone())?;
                            }
                            Expr::Assign(_, _) => {
                                evaluate_binding_target_with_value(mc, env, elem_expr, val_at.clone())?;
                            }
                            Expr::Spread(spread_expr) => {
                                let rest_obj = crate::js_array::create_array(mc, env)?;
                                let len = if let Some(len_cell) = object_get_key_value(&obj, "length") {
                            ...
                            other => {
                                evaluate_binding_target_with_value(mc, env, other, val_at.clone())?;
                            }
                        }
                    }
                }
        ...
        Expr::Object(properties) => {
            ...
            for (key_expr, target_expr, is_spread) in properties.iter() {
                ...
                match target_expr {
                    Expr::Assign(_, _) => {
                        evaluate_binding_target_with_value(mc, env, target_expr, rhs_value.clone())?;
                    }
                    Expr::Var(name, _, _) => {
                        env_set(mc, env, name, rhs_value.clone())?;
                    }
                    other => {
                        evaluate_binding_target_with_value(mc, env, other, rhs_value.clone())?;
                    }
                }
            }
            Ok(val)
            ...
            let new_val = match (current, val) {
                ...
                (Value::BigInt(_), other) | (other, Value::BigInt(_)) => {
                    if matches!(other, Value::String(_)) {
                        return Err(raise_type_error!("Cannot mix BigInt and other types in +=").into());
                    }
                ...
                _ => return Err(raise_eval_error!("AddAssign types invalid").into()),
            };
...
fn evaluate_expr_sub_assign<'gc>(
    ...
    if let Expr::Var(name, _, _) = target {
        ...
        match (current, val) {
            ...
            (l, r) => {
                ...
                if ln.is_nan() || rn.is_nan() {
                    return Err(raise_eval_error!("Invalid operands for subtraction").into());
                }
        ...
        Err(raise_eval_error!("SubAssign only for variables").into())
    }
}
    ...
    match target {
        Expr::Var(name, _, _) => {
            ...
            match (current, val) {
                ...
                (l, r) => {
                    ...
                    let rn = to_number_with_env(mc, env, &r)?;
                    if ln.is_nan() || rn.is_nan() {
                        return Err(raise_eval_error!("Invalid operands for multiplication").into());
                    }
                    let new_val = Value::Number(ln * rn);
                    env_set_recursive(mc, env, name, new_val.clone())?;
                    Ok(new_val)
        ...
        Expr::Index(obj_expr, key_expr) => {
            ...
            if let Value::Object(obj) = obj_val {
                ...
                set_property_with_accessors(mc, env, &obj, &key_str, new_val.clone())?;
                Ok(new_val)
            } else {
                Err(raise_type_error!("Cannot assign to property of non-object").into())
            }
    ...
    if let Expr::Var(name, _, _) = target {
        ...
        let new_val = match (current, val) {
            ...
            (l, r) => {
                ...
                let new = ln / denom;
                if new.is_nan() {
                    return Err(raise_eval_error!("Invalid operands for division").into());
                }
                Value::Number(new)
            }
        };
        ...
        Err(raise_eval_error!("DivAssign only for variables").into())
    }
}
    ...
    if let Expr::Var(name, _, _) = target {
        ...
        let new_val = match (current, val) {
            ...
            (l, r) => {
                ...
                let res = ln % denom;
                if res.is_nan() {
                    return Err(raise_eval_error!("Invalid operands for modulo").into());
                }
                Value::Number(res)
            }
        };
        ...
        Err(raise_eval_error!("ModAssign only for variables").into())
    }
}
...
fn evaluate_expr_pow_assign<'gc>(
    ...
    if let Expr::Var(name, _, _) = target {
        ...
        Err(raise_eval_error!("PowAssign only for variables").into())
    }
}
...
pub fn evaluate_call_dispatch<'gc>(
    ...
    match func_val {
        ...
        Value::AsyncGeneratorFunction(_, cl) => Ok(handle_async_generator_function_call(mc, &cl, &eval_args)?),
        Value::Function(name) => {
            ...
            if name == "eval" {
                ...
                let this_v = this_val.clone().unwrap_or(Value::Undefined);
                Ok(crate::js_typedarray::handle_typedarray_method(
        ...
        Value::Object(obj) => {
            if let Some(cl_ptr) = obj.borrow().get_closure() {
                match &*cl_ptr.borrow() {
                    ...
                    Value::AsyncClosure(cl) => Ok(handle_async_closure_call(mc, cl, this_val.clone(), &eval_args, env, Some(obj))?),
                    Value::GeneratorFunction(_, cl) => Ok(handle_generator_function_call(mc, cl, &eval_args, this_val.clone())?),
    ...
    match target {
        ...
        Expr::Property(obj_expr, key_str) => {
            ...
            if let Value::Object(obj) = obj_val {
                ...
                if should_assign {
                    let val = evaluate_expr(mc, env, value_expr)?;
                    set_property_with_accessors(mc, env, &obj, key_str, val.clone())?;
                    Ok(val)
                } else {
                    Ok(current)
                }
            ...
            let promise_resolve = if let Some(ctor) = crate::core::env_get(env, "Promise") {
                if let Some(resolve_method) = object_get_key_value(
                    &match ctor.borrow().clone() {
                        Value::Object(o) => o,
                        _ => return Err(raise_eval_error!("Promise not object").into()),
                    },
                    "resolve",
                ) {
                    resolve_method.borrow().clone()
                } else {
                    return Err(raise_eval_error!("Promise.resolve missing").into());
                }
            } else {
                return Err(raise_eval_error!("Promise not found").into());
            };
...
pub fn call_native_function<'gc>(
    ...
    {
        let this_v = this_val.clone().unwrap_or(Value::Undefined);
        return Ok(Some(crate::js_typedarray::handle_typedarray_method(
        ...
        match param {
            ...
            DestructuringElement::RestPattern(inner) => {
                ...
                match &**inner {
                    ...
                    DestructuringElement::NestedArray(inner_pattern, _) => {
                        bind_array_inner_for_letconst(mc, &param_env, inner_pattern, &array_obj, false, None, None)?;
                    }
                    DestructuringElement::NestedObject(inner_pattern, _) => {
                        bind_object_inner_for_letconst(mc, &param_env, inner_pattern, &array_obj, false)?;
                    }
                    _ => return Err(raise_syntax_error!("Invalid rest binding pattern").into()),
                }
            }
            DestructuringElement::NestedArray(inner_pattern, inner_default) => {
                ...
                // Use binding initialization helper so Uninitialized bindings can be initialized
                evaluate_binding_target_with_value(mc, &param_env, &pattern_with_default, arg_val)?;
            }
            DestructuringElement::NestedObject(inner_pattern, inner_default) => {
                let mut arg_val = args.get(i).cloned().unwrap_or(Value::Undefined);
                if matches!(arg_val, Value::Undefined) || matches!(arg_val, Value::Null) {
        ...
        Value::Object(obj) => {
            if let Some(cl_ptr) = obj.borrow().get_closure() {
                match &*cl_ptr.borrow() {
                    ...
                    _ => Err(raise_eval_error!("Not a constructor").into()),
                }
            } else if obj.borrow().class_def.is_some() {
