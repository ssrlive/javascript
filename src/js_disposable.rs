...
fn create_builtin_method<'gc>(
    ...
    object_set_key_value(ctx, &fn_obj, "length", &Value::Number(length as f64))?;
    fn_obj.borrow_mut(ctx).set_non_enumerable("length");
    fn_obj.borrow_mut(ctx).set_non_writable("length");
    object_set_key_value(ctx, &fn_obj, "name", &Value::String(utf8_to_utf16(display_name)))?;
    fn_obj.borrow_mut(ctx).set_non_enumerable("name");
    fn_obj.borrow_mut(ctx).set_non_writable("name");
...
pub fn initialize_disposable_stack<'gc>(ctx: &GcContext<'gc>, env: &JSObjectDataPtr<'gc>) -> Result<(), JSError> {
    ...
    object_set_key_value(ctx, &ctor, "length", &Value::Number(0.0))?;
    ctor.borrow_mut(ctx).set_non_enumerable("length");
    ctor.borrow_mut(ctx).set_non_writable("length");
    object_set_key_value(ctx, &ctor, "name", &Value::String(utf8_to_utf16("DisposableStack")))?;
    ctor.borrow_mut(ctx).set_non_enumerable("name");
    ctor.borrow_mut(ctx).set_non_writable("name");
    ...
    if let Some(op) = object_proto {
        proto.borrow_mut(ctx).prototype = Some(op);
    }
    ...
    for &(method, arity) in &[("use", 1), ("adopt", 2), ("defer", 1), ("dispose", 0), ("move", 0)] {
        ...
        let fn_obj = create_builtin_method(ctx, env, &fn_name, arity, method)?;
        if method == "dispose" {
            dispose_fn_obj = Some(fn_obj);
        }
        object_set_key_value(ctx, &proto, method, &Value::Object(fn_obj))?;
        proto.borrow_mut(ctx).set_non_enumerable(method);
    }

    // `disposed` getter — needs to be a function object with name "get disposed"
    let disposed_getter = create_builtin_method(ctx, env, "DisposableStack.prototype.disposed", 0, "get disposed")?;
    let disposed_prop = Value::Property {
        value: None,
        getter: Some(Box::new(Value::Object(disposed_getter))),
        setter: None,
    };
    object_set_key_value(ctx, &proto, "disposed", &disposed_prop)?;
    proto.borrow_mut(ctx).set_non_enumerable("disposed");
        ...
        && let Value::Object(sym_obj) = &*sym_ctor.borrow()
    {
            ...
            && let Value::Symbol(s) = &*dispose_sym.borrow()
        {
            ...
            let desc = crate::core::create_descriptor_object(ctx, &dispose_val, true, false, true)?;
            crate::js_object::define_property_internal(ctx, &proto, PropertyKey::Symbol(*s), &desc)?;
        }
        if let Some(tag_sym) = object_get_key_value(sym_obj, "toStringTag")
            && let Value::Symbol(s) = &*tag_sym.borrow()
        {
            let desc = crate::core::create_descriptor_object(ctx, &Value::String(utf8_to_utf16("DisposableStack")), false, false, true)?;
            crate::js_object::define_property_internal(ctx, &proto, PropertyKey::Symbol(*s), &desc)?;
        }
    }

    object_set_key_value(ctx, env, "DisposableStack", &Value::Object(ctor))?;
    Ok(())
}
...
pub fn handle_disposable_stack_constructor<'gc>(
    ...
    slot_set(ctx, &instance, InternalSlot::Kind, &Value::String(utf8_to_utf16("pending")));
    slot_set(ctx, &instance, InternalSlot::DisposableType, &Value::String(utf8_to_utf16("sync")));
    let resource_list = new_js_object_data(ctx);
    object_set_length(ctx, &resource_list, 0)?;
    slot_set(ctx, &instance, InternalSlot::DisposableResources, &Value::Object(resource_list));
    ...
    if let Some(Value::Object(nt_obj)) = new_target
        && let Some(proto) = crate::js_class::get_prototype_from_constructor(ctx, nt_obj, env, "DisposableStack")?
    {
        instance.borrow_mut(ctx).prototype = Some(proto);
    }
...
pub fn handle_disposable_stack_method<'gc>(
    ...
    match method {
        "use" => ds_use(ctx, this_val, args, env),
        "adopt" => ds_adopt(ctx, this_val, args, env),
        "defer" => ds_defer(ctx, this_val, args, env),
        "dispose" => ds_dispose(ctx, this_val, env),
        "move" => ds_move(ctx, this_val, env),
...
pub fn initialize_async_disposable_stack<'gc>(ctx: &GcContext<'gc>, env: &JSObjectDataPtr<'gc>) -> Result<(), JSError> {
    let ctor = new_js_object_data(ctx);
    slot_set(ctx, &ctor, InternalSlot::IsConstructor, &Value::Boolean(true));
    slot_set(
        ctx,
    ...
    object_set_key_value(ctx, &ctor, "name", &Value::String(utf8_to_utf16("AsyncDisposableStack")))?;
    ctor.borrow_mut(ctx).set_non_enumerable("name");
    ctor.borrow_mut(ctx).set_non_writable("name");
    ...
    let disposed_getter = create_builtin_method(ctx, env, "AsyncDisposableStack.prototype.disposed", 0, "get disposed")?;
    let disposed_prop = Value::Property {
        value: None,
        getter: Some(Box::new(Value::Object(disposed_getter))),
        setter: None,
        ...
        && let Value::Object(sym_obj) = &*sym_ctor.borrow()
    {
        ...
        {
            let desc = crate::core::create_descriptor_object(ctx, &Value::Object(da_fn), true, false, true)?;
            crate::js_object::define_property_internal(ctx, &proto, PropertyKey::Symbol(*s), &desc)?;
        }
        if let Some(tag_sym) = object_get_key_value(sym_obj, "toStringTag")
            && let Value::Symbol(s) = &*tag_sym.borrow()
        {
            let desc =
                crate::core::create_descriptor_object(ctx, &Value::String(utf8_to_utf16("AsyncDisposableStack")), false, false, true)?;
            crate::js_object::define_property_internal(ctx, &proto, PropertyKey::Symbol(*s), &desc)?;
        }
    }

    object_set_key_value(ctx, env, "AsyncDisposableStack", &Value::Object(ctor))?;
    Ok(())
}
...
pub fn handle_async_disposable_stack_constructor<'gc>(
    ...
    let instance = new_js_object_data(ctx);
    slot_set(ctx, &instance, InternalSlot::Kind, &Value::String(utf8_to_utf16("pending")));
    slot_set(ctx, &instance, InternalSlot::DisposableType, &Value::String(utf8_to_utf16("async")));
    let resource_list = new_js_object_data(ctx);
    object_set_length(ctx, &resource_list, 0)?;
    slot_set(ctx, &instance, InternalSlot::DisposableResources, &Value::Object(resource_list));
    ...
    if let Some(Value::Object(nt_obj)) = new_target
        && let Some(proto) = crate::js_class::get_prototype_from_constructor(ctx, nt_obj, env, "AsyncDisposableStack")?
    {
        instance.borrow_mut(ctx).prototype = Some(proto);
    }
    Ok(Value::Object(instance))
...
pub fn handle_async_disposable_stack_method<'gc>(
    ...
    match method {
        "use" => ads_use(ctx, this_val, args, env),
        "adopt" => ads_adopt(ctx, this_val, args, env),
        "defer" => ads_defer(ctx, this_val, args, env),
        "disposeAsync" => ads_dispose_async(ctx, this_val, env),
        "move" => ads_move(ctx, this_val, env),
...
fn ds_use<'gc>(
    ...
    let method = get_symbol_dispose_method(ctx, env, &value)?;
    let list = resource_list(&obj)?;
    let entry = new_js_object_data(ctx);
    object_set_key_value(ctx, &entry, "__kind", &Value::String(utf8_to_utf16("use")))?;
    slot_set(ctx, &entry, InternalSlot::PrimitiveValue, &value);
    object_set_key_value(ctx, &entry, "__dispose_method", &method)?;
    append_entry(ctx, &list, &entry)?;
    Ok(value)
}
...
fn ds_adopt<'gc>(
    ...
    let list = resource_list(&obj)?;
    let entry = new_js_object_data(ctx);
    object_set_key_value(ctx, &entry, "__kind", &Value::String(utf8_to_utf16("adopt")))?;
    slot_set(ctx, &entry, InternalSlot::PrimitiveValue, &value);
    object_set_key_value(ctx, &entry, "__callback", &on_dispose)?;
    append_entry(ctx, &list, &entry)?;
    Ok(value)
}
...
fn ds_defer<'gc>(
    ...
    let list = resource_list(&obj)?;
    let entry = new_js_object_data(ctx);
    object_set_key_value(ctx, &entry, "__kind", &Value::String(utf8_to_utf16("defer")))?;
    object_set_key_value(ctx, &entry, "__callback", &on_dispose)?;
    append_entry(ctx, &list, &entry)?;
    Ok(Value::Undefined)
}

fn ds_dispose<'gc>(ctx: &GcContext<'gc>, this_val: Option<&Value<'gc>>, env: &JSObjectDataPtr<'gc>) -> Result<Value<'gc>, EvalError<'gc>> {
    let obj = require_internal(this_val, "sync")?;
    if is_disposed(&obj) {
        return Ok(Value::Undefined);
    }
    slot_set(ctx, &obj, InternalSlot::Kind, &Value::String(utf8_to_utf16("disposed")));
    dispose_list(ctx, &obj, env, false)?;
    Ok(Value::Undefined)
}

fn ds_move<'gc>(ctx: &GcContext<'gc>, this_val: Option<&Value<'gc>>, env: &JSObjectDataPtr<'gc>) -> Result<Value<'gc>, EvalError<'gc>> {
    let obj = require_internal(this_val, "sync")?;
    require_not_disposed(&obj)?;

    let new_inst = new_js_object_data(ctx);
    slot_set(ctx, &new_inst, InternalSlot::Kind, &Value::String(utf8_to_utf16("pending")));
    slot_set(ctx, &new_inst, InternalSlot::DisposableType, &Value::String(utf8_to_utf16("sync")));

    // Steal the resource list
    if let Some(cell) = slot_get(&obj, &InternalSlot::DisposableResources) {
        slot_set(ctx, &new_inst, InternalSlot::DisposableResources, &cell.borrow().clone());
    }
    let empty = new_js_object_data(ctx);
    object_set_length(ctx, &empty, 0)?;
    slot_set(ctx, &obj, InternalSlot::DisposableResources, &Value::Object(empty));
    slot_set(ctx, &obj, InternalSlot::Kind, &Value::String(utf8_to_utf16("disposed")));
    ...
    if let Some(ctor_val) = env_get(env, "DisposableStack")
        ...
        && let Value::Object(proto) = &*proto_val.borrow()
    {
        new_inst.borrow_mut(ctx).prototype = Some(*proto);
    }
...
fn ads_use<'gc>(
    ...
    let method = get_symbol_async_dispose_or_dispose_method(ctx, env, &value)?;
    let list = resource_list(&obj)?;
    let entry = new_js_object_data(ctx);
    object_set_key_value(ctx, &entry, "__kind", &Value::String(utf8_to_utf16("use")))?;
    object_set_key_value(ctx, &entry, "__is_async", &Value::Boolean(true))?;
    slot_set(ctx, &entry, InternalSlot::PrimitiveValue, &value);
    object_set_key_value(ctx, &entry, "__dispose_method", &method)?;
    append_entry(ctx, &list, &entry)?;
    Ok(value)
...
fn ads_dispose_async<'gc>(
    ...
    let obj = require_internal(this_val, "async")?;
    if is_disposed(&obj) {
        return create_resolved_promise(ctx, env, &Value::Undefined);
    }
    slot_set(ctx, &obj, InternalSlot::Kind, &Value::String(utf8_to_utf16("disposed")));
    match dispose_list(ctx, &obj, env, true) {
        Ok(()) => create_resolved_promise(ctx, env, &Value::Undefined),
        Err(EvalError::Throw(val, _, _)) => create_rejected_promise(ctx, env, &val),
        Err(other) => Err(other),
    }
}

fn ads_move<'gc>(ctx: &GcContext<'gc>, this_val: Option<&Value<'gc>>, env: &JSObjectDataPtr<'gc>) -> Result<Value<'gc>, EvalError<'gc>> {
    let obj = require_internal(this_val, "async")?;
    require_not_disposed(&obj)?;

    let new_inst = new_js_object_data(ctx);
    slot_set(ctx, &new_inst, InternalSlot::Kind, &Value::String(utf8_to_utf16("pending")));
    slot_set(ctx, &new_inst, InternalSlot::DisposableType, &Value::String(utf8_to_utf16("async")));
    if let Some(cell) = slot_get(&obj, &InternalSlot::DisposableResources) {
        slot_set(ctx, &new_inst, InternalSlot::DisposableResources, &cell.borrow().clone());
    }
    let empty = new_js_object_data(ctx);
...
fn append_entry<'gc>(ctx: &GcContext<'gc>, list: &JSObjectDataPtr<'gc>, entry: &JSObjectDataPtr<'gc>) -> Result<(), EvalError<'gc>> {
    let len = object_get_length(list).unwrap_or(0);
    object_set_key_value(ctx, list, len, &Value::Object(*entry))?;
    object_set_length(ctx, list, len + 1)?;
    Ok(())
}
    ...
    for i in (0..len).rev() {
        ...
        let result = match kind.as_str() {
            "use" => {
                ...
                if let Some(method_cell) = object_get_key_value(&entry, "__dispose_method") {
                    ...
                    crate::core::evaluate_call_dispatch(ctx, env, &method, Some(&resource), &[])
                } else {
                    call_dispose_on_value(ctx, env, &resource, try_async || is_async_entry)
                }
            }
            "adopt" => {
                let resource = slot_get(&entry, &InternalSlot::PrimitiveValue)
                ...
                let cb = object_get_key_value(&entry, "__callback")
                    .map(|c| c.borrow().clone())
                    .unwrap_or(Value::Undefined);
                crate::core::evaluate_call_dispatch(ctx, env, &cb, None, &[resource])
            }
            "defer" => {
                let cb = object_get_key_value(&entry, "__callback")
                    .map(|c| c.borrow().clone())
                    .unwrap_or(Value::Undefined);
                crate::core::evaluate_call_dispatch(ctx, env, &cb, None, &[])
            }
            _ => continue,
        };
        ...
        if let Err(e) = result {
            ...
            completion_error = Some(match completion_error.take() {
                Some(prev) => crate::core::create_suppressed_error_value(ctx, env, &new_err, &prev),
                None => new_err,
            });
        }
    }
...
fn get_symbol_dispose_method<'gc>(
    ...
    if let Value::Symbol(sym_data) = &*dispose_sym.borrow() {
        let method = lookup_symbol_on_value(ctx, env, value, sym_data)?;
...
fn get_symbol_async_dispose_or_dispose_method<'gc>(
    ...
    get_symbol_dispose_method(ctx, env, value)
}
...
fn call_dispose_on_value<'gc>(
    ...
    let method = if try_async {
        get_symbol_async_dispose_or_dispose_method(ctx, env, value)?
    } else {
        get_symbol_dispose_method(ctx, env, value)?
    };
    crate::core::evaluate_call_dispatch(ctx, env, &method, Some(value), &[])
}
...
fn lookup_symbol_on_value<'gc>(
    ...
    match value {
        Value::Object(obj) => crate::core::get_property_with_accessors(ctx, env, obj, sym_data),
        _ => crate::core::get_primitive_prototype_property(ctx, env, value, sym_data),
    }
}
...
fn create_resolved_promise<'gc>(
        ...
        && let Some(resolve_cell) = object_get_key_value(ctor_obj, "resolve")
    {
        ...
        return crate::core::evaluate_call_dispatch(ctx, env, &resolve_fn, Some(&promise_val), std::slice::from_ref(value));
    }
...
fn create_rejected_promise<'gc>(
        ...
        && let Some(reject_cell) = object_get_key_value(ctor_obj, "reject")
    {
        ...
        return crate::core::evaluate_call_dispatch(ctx, env, &reject_fn, Some(&promise_val), std::slice::from_ref(reason));
    }
