...
use crate::{
    core::{
        JSObjectDataPtr, Value, env_set, new_js_object_data, object_get_key_value,
        object_set_key_value,
    },
    error::JSError, unicode::utf8_to_utf16,
};
...
pub(crate) fn handle_fr_constructor<'gc>(
    ...
    if !is_callable {
        return Err(
            raise_type_error!("FinalizationRegistry: cleanup must be a function").into(),
        );
    }
    ...
    if let Some(Value::Object(nt_obj)) = new_target
        && let Some(proto) = crate::js_class::get_prototype_from_constructor(
            ctx,
            nt_obj,
            env,
...
pub fn initialize_finalization_registry<'gc>(
    ctx: &GcContext<'gc>,
    env: &JSObjectDataPtr<'gc>,
) -> Result<(), JSError> {
...
pub(crate) fn handle_fr_instance_method<'gc>(
    ...
    if slot_get_chained(obj, &InternalSlot::FRMarker).is_none() {
        return Err(
            raise_type_error!(
                format!("Method FinalizationRegistry.prototype.{} called on incompatible receiver",
                method)
            )
                .into(),
        );
    ...
    match method {
        "register" => {
            ...
            if !matches!(unregister_token, Value::Undefined)
                && WeakKey::from_value(&unregister_token).is_err()
            {
                return Err(
                    raise_type_error!(
        ...
        _ => {
            Err(
                raise_type_error!(
                    format!("FinalizationRegistry.prototype.{} is not a function",
                    method)
                )
                    .into(),
            )
...
fn strict_equal<'gc>(a: &Value<'gc>, b: &Value<'gc>) -> bool {
    match (a, b) {
        ...
        (Value::Object(x), Value::Object(y)) => {
            std::ptr::eq(&*x.borrow() as *const _, &*y.borrow() as *const _)
        }
