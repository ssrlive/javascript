...
pub(crate) fn handle_set_instance_method<'gc>(mc: &MutationContext<'gc>, set: &gc_arena::Gc<'gc, gc_arena::lock::RefLock<JSSet<'gc>>>, method: &str, args: &[Expr], env: &JSObjectDataPtr<'gc>) -> Result<Value<'gc>, JSError> {
    match method {
        ...
        "values" => {
            ...
            // Set length
            set_array_length(mc, &values_array, set.borrow().values.len())?;
            Ok(Value::Object(values_array))
        }
        "keys" => {
            // For Set, keys() is the same as values()
            handle_set_instance_method(mc, set, "values", args, env)
        }
        "entries" => {
            if !args.is_empty() {
            ...
            let entries_array = new_js_object_data(mc);
            for (i, value) in set.borrow().values.iter().enumerate() {
                let entry_array = new_js_object_data(mc);
            ...
            // Set length
            set_array_length(mc, &entries_array, set.borrow().values.len())?;
