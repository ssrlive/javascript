...
pub fn initialize_set<'gc>(ctx: &GcContext<'gc>, env: &JSObjectDataPtr<'gc>) -> Result<(), JSError> {
    let set_ctor = new_js_object_data(ctx);
    slot_set(ctx, &set_ctor, InternalSlot::IsConstructor, &Value::Boolean(true));
    slot_set(ctx, &set_ctor, InternalSlot::NativeCtor, &Value::String(utf8_to_utf16("Set")));

    // Set.length = 0, Set.name = "Set" (non-enumerable, non-writable, configurable)
    object_set_key_value(ctx, &set_ctor, "length", &Value::Number(0.0))?;
    set_ctor.borrow_mut(ctx).set_non_enumerable("length");
    set_ctor.borrow_mut(ctx).set_non_writable("length");
    object_set_key_value(ctx, &set_ctor, "name", &Value::String(utf8_to_utf16("Set")))?;
    set_ctor.borrow_mut(ctx).set_non_enumerable("name");
    set_ctor.borrow_mut(ctx).set_non_writable("name");
    ...
    {
        set_ctor.borrow_mut(ctx).prototype = Some(*func_proto);
    }
    ...
    let set_proto = new_js_object_data(ctx);
    if let Some(proto) = object_proto {
        set_proto.borrow_mut(ctx).prototype = Some(proto);
    }

    object_set_key_value(ctx, &set_ctor, "prototype", &Value::Object(set_proto))?;
    set_ctor.borrow_mut(ctx).set_non_enumerable("prototype");
    set_ctor.borrow_mut(ctx).set_non_writable("prototype");
    set_ctor.borrow_mut(ctx).set_non_configurable("prototype");
    object_set_key_value(ctx, &set_proto, "constructor", &Value::Object(set_ctor))?;
    ...
    for method in methods {
        object_set_key_value(ctx, &set_proto, method, &Value::Function(format!("Set.prototype.{}", method)))?;
        set_proto.borrow_mut(ctx).set_non_enumerable(method);
    }
    ...
    if let Some(values_fn) = object_get_key_value(&set_proto, "values") {
        object_set_key_value(ctx, &set_proto, "keys", &values_fn.borrow().clone())?;
        set_proto.borrow_mut(ctx).set_non_enumerable("keys");
    }
    // Mark constructor non-enumerable
    set_proto.borrow_mut(ctx).set_non_enumerable("constructor");
    ...
    object_set_key_value(ctx, &set_proto, "size", &size_prop)?;
    set_proto.borrow_mut(ctx).set_non_enumerable("size");
        ...
        && let Value::Object(sym_obj) = &*sym_ctor.borrow()
    {
            ...
            && let Value::Symbol(s) = &*iter_sym.borrow()
        {
            ...
            crate::js_object::define_property_internal(ctx, &set_proto, crate::core::PropertyKey::Symbol(*s), &iter_desc)?;
        }
        ...
        {
            let tag_desc = crate::core::create_descriptor_object(ctx, &Value::String(utf8_to_utf16("Set")), false, false, true)?;
            crate::js_object::define_property_internal(ctx, &set_proto, crate::core::PropertyKey::Symbol(*s), &tag_desc)?;
        }
            ...
            && let Value::Symbol(s) = &*species_sym.borrow()
        {
            ...
            crate::js_object::define_property_internal(ctx, &set_ctor, crate::core::PropertyKey::Symbol(*s), &species_desc_obj)?;
        }
    }
    ...
    env_set(ctx, env, "Set", &Value::Object(set_ctor))?;

    // --- %SetIteratorPrototype% ---
    // [[Prototype]] = %IteratorPrototype%
    let set_iter_proto = new_js_object_data(ctx);
    ...
    {
        set_iter_proto.borrow_mut(ctx).prototype = Some(*iter_proto);
    }
    ...
    set_iter_proto.borrow_mut(ctx).set_non_enumerable("next");
        ...
        && let Value::Object(sym_obj) = &*sym_ctor.borrow()
    {
        ...
        {
            let tag_desc = crate::core::create_descriptor_object(ctx, &Value::String(utf8_to_utf16("Set Iterator")), false, false, true)?;
            crate::js_object::define_property_internal(ctx, &set_iter_proto, crate::core::PropertyKey::Symbol(*tag_sym), &tag_desc)?;
        }
    }

    slot_set(ctx, env, InternalSlot::SetIteratorPrototype, &Value::Object(set_iter_proto));
...
pub(crate) fn handle_set_constructor<'gc>(
    ...
    let set = new_gc_cell_ptr(ctx, JSSet { values: Vec::new() });

    // Create a wrapper object for the Set
    let set_obj = new_js_object_data(ctx);
    // Store the actual set data
    slot_set(ctx, &set_obj, InternalSlot::Set, &Value::Set(set));

    // OrdinaryCreateFromConstructor(NewTarget, "%Set.prototype%")
    let mut proto_set = false;
    if let Some(Value::Object(nt_obj)) = new_target
        && let Some(proto) = crate::js_class::get_prototype_from_constructor(ctx, nt_obj, env, "Set")?
    {
        set_obj.borrow_mut(ctx).prototype = Some(proto);
        proto_set = true;
    }
    ...
    {
        set_obj.borrow_mut(ctx).prototype = Some(*proto_obj);
    }
    ...
    let add_fn = crate::core::get_property_with_accessors(ctx, env, &set_obj, "add")?;
    ...
    loop {
        ...
        // Call(adder, set, [item])
        let call_result = crate::core::evaluate_call_dispatch(ctx, env, &add_fn, Some(&Value::Object(set_obj)), &[item]);
...
pub(crate) fn handle_set_instance_method<'gc>(
    ...
    match method {
        "add" => {
            ...
            if !exists {
                set.borrow_mut(ctx).values.push(Some(value));
            }
        ...
        "delete" => {
            ...
            for entry in set.borrow_mut(ctx).values.iter_mut() {
                if let Some(v) = entry
        ...
        "clear" => {
            set.borrow_mut(ctx).values.clear();
            Ok(Value::Undefined)
        }
        "size" => Ok(Value::Number(set.borrow().values.iter().filter(|e| e.is_some()).count() as f64)),
        "keys" => Ok(create_set_iterator(ctx, env, *set, "values")?), // Set keys === values
        "values" => Ok(create_set_iterator(ctx, env, *set, "values")?),
        "entries" => Ok(create_set_iterator(ctx, env, *set, "entries")?),
        "forEach" => {
            if args.is_empty() {
        ...
        "union" | "intersection" | "difference" | "symmetricDifference" | "isSubsetOf" | "isSupersetOf" | "isDisjointFrom" => {
            handle_set_method(ctx, set, method, args, env, this_obj)
        }
...
fn get_set_record<'gc>(
    ...
    let raw_size = crate::core::get_property_with_accessors(ctx, env, &obj, "size")?;
    if matches!(raw_size, Value::Undefined) {
        return Err(raise_type_error!("other.size is undefined").into());
    }
    let size_prim = crate::core::to_primitive(ctx, &raw_size, "number", env)?;
    ...
    // 2. Get has method
    let has_fn = crate::core::get_property_with_accessors(ctx, env, &obj, "has")?;
    if !is_callable(&has_fn) {
        return Err(raise_type_error!("other.has is not a function").into());
    }

    // 3. Get keys method
    let keys_fn = crate::core::get_property_with_accessors(ctx, env, &obj, "keys")?;
...
{
    let keys_result = crate::core::evaluate_call_dispatch(ctx, env, keys_fn, Some(&Value::Object(*other_obj)), &[])?;
    ...
    loop {
        let result = crate::js_map::call_iterator_next(ctx, env, &iter_obj, &next_fn)?;
        let done = crate::js_map::get_iterator_done(ctx, env, &result)?;
        if done {
            break;
        }
        let value = crate::js_map::get_iterator_value(ctx, env, &result)?;
        let cont = callback(ctx, value)?;
        if !cont {
...
fn handle_set_method<'gc>(
    ...
    let (other_obj, has_fn, keys_fn, _other_size) = get_set_record(ctx, env, &other_arg)?;

    match method {
        "union" => {
            // Create new Set, copy all this entries, then add all other entries
            let result_set = new_gc_cell_ptr(ctx, JSSet { values: Vec::new() });
            // Copy this set's values
            for v in set.borrow().values.iter().flatten() {
                result_set.borrow_mut(ctx).values.push(Some(v.clone()));
            }
            // Add values from other via keys() iterator
            iterate_other_keys(ctx, env, &other_obj, &keys_fn, |ctx, value| {
                ...
                if !exists {
                    result_set.borrow_mut(ctx).values.push(Some(normalized));
                }
                Ok(true)
            })?;
            Ok(wrap_set_as_object(ctx, env, result_set)?)
        }
        "intersection" => {
            let result_set = new_gc_cell_ptr(ctx, JSSet { values: Vec::new() });
                ...
                iterate_other_keys(ctx, env, &other_obj, &keys_fn, |ctx, value| {
                    ...
                    if in_this {
                        ...
                        if !already_in_result {
                            result_set.borrow_mut(ctx).values.push(Some(normalize_set_value(normalized)));
                        }
                    }
                    Ok(true)
                })?;
        ...
        "difference" => {
            ...
            if this_size <= _other_size {
                ...
                iterate_other_keys(ctx, env, &other_obj, &keys_fn, |ctx, value| {
                    ...
                    for entry in result_ref.borrow_mut(ctx).values.iter_mut() {
                        if let Some(v) = entry
        ...
        "symmetricDifference" => {
            ...
            iterate_other_keys(ctx, env, &other_obj, &keys_fn, |ctx, value| {
                ...
                if in_live {
                    ...
                    for entry in result_set.borrow_mut(ctx).values.iter_mut() {
                        if let Some(v) = entry
                            && same_value_zero(v, &normalized)
                        {
                            *entry = None;
        ...
        "isSubsetOf" => {
            ...
            loop {
                ...
                if let Some(v) = entry {
                    let in_other =
                        crate::core::evaluate_call_dispatch(ctx, env, &has_fn, Some(&Value::Object(other_obj)), std::slice::from_ref(&v))?;
                    if !in_other.to_truthy() {
                        return Ok(Value::Boolean(false));
                    }
                }
        ...
        "isSupersetOf" => {
            ...
            let set_ref = *set;
            let mut result = true;
            iterate_other_keys(ctx, env, &other_obj, &keys_fn, |_ctx, value| {
                let normalized = normalize_set_value(value);
                let in_this = set_ref
                    .borrow()
                    .values
...
fn wrap_set_as_object<'gc>(
    ...
    slot_set(ctx, &obj, InternalSlot::Set, &Value::Set(set));
...
fn create_set_iterator<'gc>(
    ...
    slot_set(ctx, &iterator, InternalSlot::IteratorSet, &Value::Set(set));
...
pub(crate) fn handle_set_iterator_next<'gc>(
    ...
    if index >= values.len() {
        ...
        slot_set(ctx, iterator, InternalSlot::IteratorSet, &Value::Undefined);
        let result_obj = new_js_object_data(ctx);
        object_set_key_value(ctx, &result_obj, "value", &Value::Undefined)?;
        object_set_key_value(ctx, &result_obj, "done", &Value::Boolean(true))?;
        return Ok(Value::Object(result_obj));
    ...
    let result_value = match kind.as_str() {
        ...
        "entries" => {
            ...
            object_set_key_value(ctx, &entry_array, "0", &value.clone())?;
