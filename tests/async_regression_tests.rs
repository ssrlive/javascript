...
fn test_async_n_throw_async_tests_regression() {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            match evaluate_script_with_vm(&wrapped, false, Some(path)) {
                ...
                Err(e) => {
                    let _ = tx.send(Err(format!("evaluate_script_with_vm failed: {:?}", e)));
                }
            }
        })
    ...
    match rx.recv_timeout(timeout) {
        Ok(Ok(result)) => {
            // evaluate_script_with_vm wraps returned JS strings as JSON quoted strings, so parse twice
...
fn test_handlers_repro() {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            match evaluate_script_with_vm(&script, false, Some(path)) {
                Ok(result) => {
                    let _ = tx.send(Ok(result));
                }
                Err(e) => {
