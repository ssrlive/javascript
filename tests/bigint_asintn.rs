...
fn bigint_asintn_asuintn_basic() {
    ...
    let r1 = evaluate_script_with_vm("BigInt.asUintN(3, 7n)", None::<&std::path::Path>);
    match r1 {
        Ok(h) => assert_eq!(h, "7"),
    ...
    let r2 = evaluate_script_with_vm("BigInt.asIntN(3, 7n)", None::<&std::path::Path>);
    match r2 {
        Ok(h) => assert_eq!(h, "-1"),
    ...
    // bits == 0
    let r3 = evaluate_script_with_vm("BigInt.asUintN(0, 123n)", None::<&std::path::Path>);
    match r3 {
        Ok(h) => assert_eq!(h, "0"),
        other => panic!("expected BigInt result for asUintN bits=0, got {:?}", other),
    }

    let r4 = evaluate_script_with_vm("BigInt.asIntN(0, -5n)", None::<&std::path::Path>);
    match r4 {
        Ok(h) => assert_eq!(h, "0"),
    ...
    let r5 = evaluate_script_with_vm("BigInt.asUintN(4, -1n)", None::<&std::path::Path>);
    match r5 {
        Ok(h) => assert_eq!(h, "15"),
    ...
    let r6 = evaluate_script_with_vm("BigInt.asIntN(4, -1n)", None::<&std::path::Path>);
    match r6 {
        Ok(h) => assert_eq!(h, "-1"),
    ...
    let r7 = evaluate_script_with_vm("BigInt.asIntN(4, 8n)", None::<&std::path::Path>);
    match r7 {
        Ok(h) => assert_eq!(h, "-8"),
    ...
    let r8 = evaluate_script_with_vm("BigInt.asUintN(64, 18446744073709551616n)", None::<&std::path::Path>);
    match r8 {
        Ok(h) => assert_eq!(h, "0"),
    ...
    let r9 = evaluate_script_with_vm("BigInt.asIntN(64, 9223372036854775808n)", None::<&std::path::Path>);
    match r9 {
...
fn bigint_asintn_conversion_cases() {
    // String input
    let r1 = evaluate_script_with_vm("BigInt.asUintN(5, '15')", None::<&std::path::Path>);
    match r1 {
        Ok(h) => assert_eq!(h, "15"),
    ...
    let r2 = evaluate_script_with_vm("BigInt.asUintN(3, 7.0)", None::<&std::path::Path>);
    assert!(r2.is_err(), "expected TypeError for numeric input to ToBigInt");

    // Number input (non-integer) should also fail
    let r3 = evaluate_script_with_vm("BigInt.asUintN(3, 7.5)", None::<&std::path::Path>);
    assert!(r3.is_err(), "expected TypeError for non-integer numeric input");

    // Boolean inputs
    let r4 = evaluate_script_with_vm("BigInt.asUintN(4, true)", None::<&std::path::Path>);
    match r4 {
        Ok(h) => assert_eq!(h, "1"),
        other => panic!("expected BigInt result for asUintN boolean true, got {:?}", other),
    }
    let r5 = evaluate_script_with_vm("BigInt.asUintN(4, false)", None::<&std::path::Path>);
    match r5 {
        Ok(h) => assert_eq!(h, "0"),
    ...
    let r6 = evaluate_script_with_vm("BigInt.asUintN(4, { valueOf: function(){ return 7n; } })", None::<&std::path::Path>);
    match r6 {
        Ok(h) => assert_eq!(h, "7"),
    ...
    let r6b = evaluate_script_with_vm("BigInt.asUintN(4, { valueOf: function(){ return 7; } })", None::<&std::path::Path>);
    ...
    let r7 = evaluate_script_with_vm("BigInt.asUintN(3.5, 7n)", None::<&std::path::Path>);
    match r7 {
        Ok(h) => assert_eq!(h, "7"),
