...
fn bigint_vs_number_comparison_edgecases() {
    // 1n == 1
    let r1 = evaluate_script("1n == 1", false, None::<&std::path::Path>);
    match r1 {
    ...
    // 1n == 1.5 -> false
    let r2 = evaluate_script("1n == 1.5", false, None::<&std::path::Path>);
    match r2 {
    ...
    let r3 = evaluate_script("9007199254740991n == 9007199254740991", false, None::<&std::path::Path>);
    match r3 {
        Ok(b) => assert_eq!(b, "true"),
    ...
    let r3b = evaluate_script("123456789123456789n == 123456789123456789", false, None::<&std::path::Path>);
    match r3b {
        Ok(b) => assert_eq!(b, "false"),
    ...
    let r4 = evaluate_script("1n < 1.5", false, None::<&std::path::Path>);
    match r4 {
        Ok(b) => assert_eq!(b, "true"),
        other => panic!("unexpected result for 1n < 1.5: {:?}", other),
    }
    ...
    let r5 = evaluate_script("123456789123456789123456789n > 1e20", false, None::<&std::path::Path>);
    match r5 {
        Ok(b) => assert_eq!(b, "true"),
    ...
    let r6 = evaluate_script("(0n - 123456789123456789123456789n) < -1e20", false, None::<&std::path::Path>);
    match r6 {
        Ok(b) => assert_eq!(b, "true"),
