...
fn bigint_addition_and_mixing() {
    ...
    let res = evaluate_script_with_vm("1n + 2n", None::<&std::path::Path>);
    match res {
        Ok(h) => assert_eq!(h, "3"),
    ...
    let mix = evaluate_script_with_vm("1n + 1", None::<&std::path::Path>);
    assert!(mix.is_err());

    // Loose equality between BigInt and Number (1n == 1) — per spec this should be true
    let eq = evaluate_script_with_vm("1n == 1", None::<&std::path::Path>);
    match eq {
