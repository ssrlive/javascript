...
fn bigint_arithmetic_and_mixing_should_behave_per_spec() {
    ...
    let r1 = evaluate_script("1n + 2n", false, None::<&std::path::Path>);
    match r1 {
        Ok(h) => assert!(h == "3"),
    ...
    let r2 = evaluate_script("1n + 1", false, None::<&std::path::Path>);
    ...
    let r3 = evaluate_script("5n - 2", false, None::<&std::path::Path>);
    assert!(r3.is_err(), "Expected TypeError for 5n - 2");

    // Loose equality: 1n == 1 should be true
    let r4 = evaluate_script("1n == 1", false, None::<&std::path::Path>);
    match r4 {
    ...
    let r5 = evaluate_script("1n === 1", false, None::<&std::path::Path>);
    match r5 {
    ...
    let r6 = evaluate_script("2n > 1", false, None::<&std::path::Path>);
    match r6 {
        Ok(b) => assert_eq!(b, "true"),
    ...
    let r7 = evaluate_script("1n < 1.5", false, None::<&std::path::Path>);
    match r7 {
        Ok(b) => assert_eq!(b, "true"),
    ...
    let r8 = evaluate_script("'x' + 1n", false, None::<&std::path::Path>);
    match r8 {
        Ok(s) => assert_eq!(s, "\"x1\""),
