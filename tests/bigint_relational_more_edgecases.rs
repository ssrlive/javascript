...
fn bigint_relational_more_edgecases() {
    ...
    let r1 = evaluate_script("9007199254740991n == 9007199254740991");
    match r1 {
    ...
    let r2 = evaluate_script("9007199254740992n == 9007199254740992");
    match r2 {
        Ok(Value::Boolean(b)) => assert!(b),
    ...
    let r3 = evaluate_script("9007199254740993n == 9007199254740993");
    match r3 {
        Ok(Value::Boolean(b)) => assert!(!b),
    ...
    let r4 = evaluate_script("(0n - 9007199254740991n) == -9007199254740991");
    match r4 {
        Ok(Value::Boolean(b)) => assert!(b),
    ...
    let r9 = evaluate_script("123456789123456789123456789n > 1e20");
    match r9.unwrap() {
        Value::Boolean(b) => assert!(b),
        other => panic!("unexpected result for huge BigInt > 1e20: {:?}", other),
    }

    let r10 = evaluate_script("(0n - 123456789123456789123456789n) < -1e20");
    match r10.unwrap() {
        Value::Boolean(b) => assert!(b),
    ...
    let r11 = evaluate_script("4.9 < 5n");
    match r11.unwrap() {
        Value::Boolean(b) => assert!(b),
    ...
    // 5.0 < 5n -> false
    let r12 = evaluate_script("5.0 < 5n");
    match r12.unwrap() {
        Value::Boolean(b) => assert!(!b),
    ...
    let r13 = evaluate_script("5.1 > 5n");
    match r13.unwrap() {
        Value::Boolean(b) => assert!(b),
