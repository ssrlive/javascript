...
fn test_logical_assignments() {
    ...
    let result1 = evaluate_script(code1, None::<&std::path::Path>).unwrap();
    assert_eq!(result1, "10");

    let code2 = "let b = 0; b &&= 10; b";
    let result2 = evaluate_script(code2, None::<&std::path::Path>).unwrap();
    assert_eq!(result2, "0");

    // Test logical OR assignment (||=)
    let code3 = "let c = 5; c ||= 10; c";
    let result3 = evaluate_script(code3, None::<&std::path::Path>).unwrap();
    assert_eq!(result3, "5");

    let code4 = "let d = 0; d ||= 10; d";
    let result4 = evaluate_script(code4, None::<&std::path::Path>).unwrap();
    assert_eq!(result4, "10");

    // Test nullish coalescing assignment (??=)
    let code5 = "let e = 5; e ??= 10; e";
    let result5 = evaluate_script(code5, None::<&std::path::Path>).unwrap();
    assert_eq!(result5, "5");

    let code6 = "let f; f ??= 10; f";
    let result6 = evaluate_script(code6, None::<&std::path::Path>).unwrap();
