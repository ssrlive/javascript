...
mod number_tests {
    ...
    fn test_shift_edge_cases_and_bigint_mixing() {
        ...
        let res = evaluate_script_with_vm("let a = 1; a <<= 33; a", None::<&std::path::Path>).unwrap();
        assert_eq!(res, "2");

        // Left shift with negative shift amount -> ToUint32(-1) & 0x1f == 31
        let res = evaluate_script_with_vm("let a = 1; a <<= -1; a", None::<&std::path::Path>).unwrap();
        ...
        let res = evaluate_script_with_vm("let a = 1n; let b = 2; a <<= b", None::<&std::path::Path>);
        match res {
        ...
        let res = evaluate_script_with_vm("let a = 1n; a >>>= 1n", None::<&std::path::Path>);
        match res {
            Err(err) => match err.kind() {
    ...
    fn test_bitwise_compound_assignments() {
        ...
        let result1 = evaluate_script_with_vm(script1, None::<&std::path::Path>).unwrap();
        assert_eq!(result1, "1");

        // Test bitwise OR assignment (|=)
        let script2 = "let b = 5; b |= 3; b";
        let result2 = evaluate_script_with_vm(script2, None::<&std::path::Path>).unwrap();
        assert_eq!(result2, "7");

        // Test bitwise XOR assignment (^=)
        let script3 = "let c = 5; c ^= 3; c";
        let result3 = evaluate_script_with_vm(script3, None::<&std::path::Path>).unwrap();
        assert_eq!(result3, "6");

        // Test left shift assignment (<<=)
        let script4 = "let d = 5; d <<= 1; d";
        let result4 = evaluate_script_with_vm(script4, None::<&std::path::Path>).unwrap();
        assert_eq!(result4, "10");

        // Test right shift assignment (>>=)
        let script5 = "let e = 5; e >>= 1; e";
        let result5 = evaluate_script_with_vm(script5, None::<&std::path::Path>).unwrap();
        assert_eq!(result5, "2");

        // Test unsigned right shift assignment (>>>=)
        let script6 = "let f = -5; f >>>= 1; f";
        let result6 = evaluate_script_with_vm(script6, None::<&std::path::Path>).unwrap();
