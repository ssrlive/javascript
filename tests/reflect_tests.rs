...
fn test_reflect_has() {
    ...
    let result = evaluate_script("Reflect.has({test: 1}, 'test')").unwrap();
    assert!(matches!(result, Value::Boolean(true)));

    // Test Reflect.has with non-existing property
    let result = evaluate_script("Reflect.has({}, 'test')").unwrap();
...
fn test_reflect_get() {
    ...
    let result = evaluate_script("Reflect.get({test: 42}, 'test')").unwrap();
    assert!(matches!(result, Value::Number(n) if (n - 42.0).abs() < f64::EPSILON));

    // Test Reflect.get with non-existing property
    let result = evaluate_script("Reflect.get({}, 'test')").unwrap();
...
fn test_reflect_set() {
    // Test Reflect.set
    let result = evaluate_script("let obj = {}; Reflect.set(obj, 'test', 123); obj.test").unwrap();
...
fn test_reflect_own_keys() {
    ...
    let result = evaluate_script("Reflect.ownKeys({a: 1, b: 2}).length").unwrap();
...
fn test_reflect_is_extensible() {
    ...
    let result = evaluate_script("Reflect.isExtensible({})").unwrap();
    assert!(matches!(result, Value::Boolean(true)));
}
...
fn test_reflect_get_prototype_of() {
    ...
    let result = evaluate_script("typeof Reflect.getPrototypeOf({})").unwrap();
    match result {
