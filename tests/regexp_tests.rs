...
mod regexp_tests {
    ...
    fn test_regexp_constructor() {
        let result = evaluate_script("new RegExp('hello')", false, None::<&std::path::Path>).unwrap();
        assert_eq!(result, "[object RegExp]");
    }

    #[test]
    fn test_regexp_constructor_with_flags() {
        let result = evaluate_script("new RegExp('hello', 'gi')", false, None::<&std::path::Path>).unwrap();
        assert_eq!(result, "[object RegExp]");
    }

    #[test]
    fn test_regexp_test_method() {
        let result = evaluate_script("new RegExp('hello').test('hello world')", false, None::<&std::path::Path>).unwrap();
        assert_eq!(result, "true");
    }

    #[test]
    fn test_regexp_test_method_case_insensitive() {
        let result = evaluate_script("new RegExp('hello', 'i').test('HELLO world')", false, None::<&std::path::Path>).unwrap();
        assert_eq!(result, "true");
    }

    #[test]
    fn test_regexp_exec_method() {
        let result = evaluate_script("new RegExp('hello').exec('hello world')[0]", false, None::<&std::path::Path>).unwrap();
    ...
    fn test_regexp_extract_emails() {
        ...
        let result = evaluate_script("new RegExp('test').test('test string')", false, None::<&std::path::Path>).unwrap();
        assert_eq!(result, "true");
    }
    ...
    fn test_regexp_to_string() {
        let result = evaluate_script("new RegExp('ab+c').toString()", false, None::<&std::path::Path>).unwrap();
    ...
    fn test_string_match_global_behavior() {
        let result = evaluate_script(r#"'cdbbdbsbz'.match(/d(b+)d/g)[0]"#, false, None::<&std::path::Path>).unwrap();
        assert_eq!(result, "\"dbbd\""); // "dbbd"
    }

    #[test]
    fn test_string_match_non_global_captures() {
        let result = evaluate_script(r#"'cdbbdbsbz'.match(/d(b+)d/)[1]"#, false, None::<&std::path::Path>).unwrap();
